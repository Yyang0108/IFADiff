...
class HSILocalControlUNetModel(UNetModel):
    def __init__(self,
        image_size=32,
        in_channels= 3,
        model_channels= 320,
        ...
        super().__init__(
            ...
            legacy=legacy)
        self.conv_hsi = nn.Conv2d(hsi_channels, model_channels, 3, 1, 1)
        self.conv_rgb = nn.Conv2d(hsi_channels, in_channels, 3, 1, 1)

        self.conv_last_hsi = nn.Conv2d(model_channels, hsi_channels, 3, 1, 1)
        self.conv_last_rgb = nn.Conv2d(out_channels, hsi_channels, 3, 1, 1)
    ...
    def forward(self, x, timesteps=None, metadata=None,context=None, local_control=None,meta=False, **kwargs):
        ...
        h_hsi = self.conv_hsi(h)
        h_rgb = self.input_blocks[0](self.conv_rgb(h), emb, context)
        h = h_rgb + h_hsi
        hs.append(h)
        ...
        for module in self.output_blocks:
            h = torch.cat([h, hs.pop() + local_control.pop()], dim=1)
            h = module(h, emb, context)
        h_rgb = self.conv_last_rgb(self.out(h))
        h_hsi = self.conv_last_hsi(h)
        h = h_rgb + h_hsi
