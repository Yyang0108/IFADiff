...
def total_variation_denoise(x, weight=0.01, steps=1):
    """
    输入: x: (B, C, H, W)
    输出: 去 TV 噪的图像（不做梯度传播）
    """
    for _ in range(steps):
        x = x - weight * F.pad(torch.sign(x[:, :, :-1, :] - x[:, :, 1:, :]), (0, 0, 0, 1)) \
              - weight * F.pad(torch.sign(x[:, :, :, :-1] - x[:, :, :, 1:]), (0, 1, 0, 0))
    return x
...
class DDIMSampler(object):
    ...
    def sample(self,
        ...
        samples, intermediates, x_T = self.ddim_sampling(conditioning, size,
                                                    metadata,
                                                    callback=callback,
    ...
    def ddim_sampling(self, cond, shape, metadata,
        ...
        for i, step in enumerate(iterator):
            ...
            img, pred_x0 = outs
            # img = total_variation_denoise(img, weight=0.01, steps=1)
    ...
    def p_sample_ddim(self, x, c, t, index,metadata, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
        ...
        # print(f"c:{c['local_control'][0].shape}")
